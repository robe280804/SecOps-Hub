...
class StopProjectEnvironment implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(EnvironmentProvisioner $provisioner): void
    {
        ...
        if ($user === null || Gate::forUser($user)->denies('stop', $environment)
            || $environment->desired_state !== EnvironmentDesiredState::Stopped) {
            DB::transaction(function () use ($operation, $environment): void {
                $operation->forceFill(['status' => 'cancelled', 'completed_at' => now()])->save();
                $environment->forceFill(['status' => EnvironmentStatus::Error, 'last_error' => 'Stop authorization is no longer valid.'])->save();
            });

            return;
        }
        $operation->forceFill(['status' => 'running', 'attempts' => $operation->attempts + 1])->save();
        $provisioner->stop($environment);
        DB::transaction(function () use ($operation, $environment): void {
            $environment->forceFill(['status' => EnvironmentStatus::Stopped, 'last_error' => null])->save();
    ...
    public function failed(?Throwable $exception): void
    {
        ...
        DB::transaction(function () use ($operation): void {
            ...
            $operation->environment->forceFill([
                ...
                'last_error' => 'Environment stop failed. Check the worker logs before retrying.',
            ])->save();
        });
    }
}
